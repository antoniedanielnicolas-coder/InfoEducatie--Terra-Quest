...
// ─────────────────────────────────────────────────────────────────────────────
// MUSIC LIBRARY — Web Audio API procedural ambient generation
// No external files, no CORS, no autoplay policy issues
// ─────────────────────────────────────────────────────────────────────────────
...
let musicNodes       = [];   // nodes to stop when switching
let musicPlaying     = false;
let musicGainNode    = null;  // separate gain for volume control
...
export function initSounds() {
    ...
    // Music gain node (separate from UI effects)
    sounds.ambient = { _procedural: true };  // placeholder so checks pass
    ...
    // Unlock AudioContext + start music on first user interaction
...
function unlockAudio() {
    // no-op: audio is started via startMusicOnce which has been removed here
}

// ─── PROCEDURAL MUSIC ENGINE ─────────────────────────────────────────────────
// Each track is a layered synth pattern generated by Web Audio API oscillators.
// No external files — works offline and without CORS restrictions.
const trackPatterns = {
...
function playMusic(trackId) {
    ...
    // Layer 1: Soft major chord pad
    const droneFreqs = [pat.baseFreq, pat.baseFreq * 1.25, pat.baseFreq * 1.5]; // Major triad: root, major third, perfect fifth
    ...
    // Layer 2: Gentle LFO tremolo on a mid oscillator
    ...
    // Layer 3: Cheerful Arpeggiator pulse
...
export function setBackgroundMusic(musicId) {
    ...
    if (track && audioCtx) {
        ...
        // AudioCtx not ready yet, just save preference
        currentMusicId = musicId;
        localStorage.setItem('currentMusicId', musicId);
    }
}
